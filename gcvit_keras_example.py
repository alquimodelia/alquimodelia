...
class WindowAttention(layers.Layer):
    ...
    def get_relative_position_index(self):
        ...
        coords = ops.stack(
            ops.meshgrid(coords_h, coords_w, indexing="ij"), axis=0
        )
    ...
    def call(self, inputs, **kwargs):
        ...
        if self.global_query:
            ...
            q = ops.reshape(
                q_global, [B_, N, self.num_heads, C // self.num_heads]
            )
        ...
        relative_position_bias = ops.transpose(
            relative_position_bias, axes=[2, 0, 1]
        )
...
class Level(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.q_global_gen = GlobalQueryGenerator(
            self.keepdims, name="q_global_gen"
        )
        super().build(input_shape)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.head = layers.Dense(
            num_classes, name="head", activation=head_activation
        )
...
def make_dataset(
    dataset: tf.data.Dataset, train: bool, image_size: int = IMAGE_SIZE
):
